...
async def prediction(file: UploadFile = File(...)):
    ...
    # return output
    # output = [word for word in output if word not in ["<start>", "<eos>"]]

    # Convert the tokenized caption to a string
    # caption_text = " ".join(output)
...
def streamlit_app():
    ...
    with st.sidebar:
        selected = option_menu("Main Menu", ["Introduction", 'Project'], 
    ...
    if selected == "Project":
        ...
        <style>
        body {
        background-image: "url("https://img.freepik.com/free-vector/hand-painted-watercolor-pastel-sky-background_23-2148901163.jpg")";
...
if __name__ == "__main__":
    ...
    fastapi_thread = threading.Thread(target=uvicorn.run, args=("main:app",), kwargs={"host": "0.0.0.0", "port": 8000, "log_level": "info", "reload": True, "workers": 1})
    fastapi_thread.start()

    # Run the Streamlit interface in the main thread
    streamlit_app()